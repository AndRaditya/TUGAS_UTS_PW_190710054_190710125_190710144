<?php
    ...
    if(isset($_POST['login'])){
        include('../db/db.php'); // untuk mengoneksikan dengan databas dengan memanggil file db.php
        ...
        if(mysqli_num_rows($query) == 0){
            echo
                '<script>
                alert("Username not found!"); window.location = "../Login_Register/loginPage.php"
                </script>';
        }else{
            ...
            if(password_verify($password, $user['password'])){
                ...
                echo
                    '<script>
                    alert("Login Success"); window.location = "../Beranda.php"
                    </script>';
            }else {
                echo
                    '<script>
                    alert("Username or Password Invalid");
                    window.location = "../Login_Register/loginPage.php"
                    </script>';
            }
        }
    }else{
